 ...
 */
public class RSFurnaceRecipeHandler extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if(outputId.equals(getOverlayIdentifier()) && this.getClass() == RSFurnaceRecipeHandler.class) {
            Map recipes = FurnaceRecipes.smelting().getSmeltingList();
            Iterator i$ = recipes.entrySet().iterator();

            while(i$.hasNext()) {
                Map.Entry recipe = (Map.Entry)i$.next();
                this.arecipes.add(new RSFurnaceRecipeHandler.SmeltingPair((ItemStack)recipe.getKey(), (ItemStack)recipe.getValue()));
            }
        } else {
    ...
    public class SmeltingPair extends CachedRecipe {
        ...
        public java.util.List<PositionedStack> getIngredients() {
            return this.getCycledIngredients(RSFurnaceRecipeHandler.this.cycleticks / 48, ingredients);
        }
